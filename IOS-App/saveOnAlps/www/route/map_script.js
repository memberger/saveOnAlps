...
maps = L.map('map',{zoomControl:false}).setView([latitude, longitude], 13);
    maps.zoomControl=true;
    L.tileLayer('http://a.tile.openstreetmap.org/{z}/{x}/{y}.png',{
         maxZoom: 18,
    }).addTo(maps);
...
if(allowmarker){
	...
	punkteBox.style.backgroundRepeat = "no-repeat";
	...
	for(var i=0; i<markers.length;i++){
		if(markers[i]!= null){
		markers[i].bindPopup("<div onclick = 'markerLöschen("+i+");'>Löschen</div>");
		}
	}
	...
	if(markercounter > 1 && markercounter <= 100){
		if(markercounter<=10){
		icon = L.divIcon({
		iconSize: [30, 37],
		iconAnchor: [15, 37],
		className: 'mapicon',
		html: "<img src='img/over.png' /> <div>"+"0"+(markercounter-1)+"</div>"
		});
		}
		else{
		icon = L.divIcon({
	...
	marker.bindPopup("<div onclick = 'markerLöschen("+markercounter+");'>Löschen</div>");
	markercounter++;
	}
}
window.scrollTo(0, 0)
...
for(var i = 0; i<markercounter; i++) {
	if(markers[i]!= null){
    coordstring.coords.push({ 
        "lat" : markers[i]._latlng.lat,
        "lon"  : markers[i]._latlng.lng,
    });
	
	}

 }
	return coordstring.coords;
}